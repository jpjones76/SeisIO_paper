...
pref  = pwd() * "/Benchmarks/"
path  = dirname(pathof(SeisIO)) * "/../test/SampleFiles/Restricted/"
test0 = String[
                "1day-1hz.ah"                   "AH1"
                "2days-40hz.h5"                 "ASDF"
                "geo-tspair.csv"                "GEOCSV"
                "1day-100hz.mseed"              "MSEED-1"
                "SHW.UW.mseed"                  "MSEED-2"
                "1day-100hz.segy"               "PASSCAL"
                "1day-100hz.sac"                "SAC"
                "1h-62.5hz.slist"               "SLIST"
                "10081701.WVP"                  "SUDS"
                "99011116541W"                  "UW"
                "2014092709*.cnt"               "WIN32"
              ]
filenames = [pref, pref, pref, pref, path, pref, pref, pref, path, pref, path].*test0[:,1]
...
# Python benchmarks
M_py = readdlm("Python/benchmarks_python.csv", ';')
N_py = size(M_py, 1)
for i = 1:N_py
  println(i)
