...
# paths and filenames
rootpath = os.getcwd()
DATADIR  = os.path.join(rootpath,'../output/%s/obspy/RAW_DATA_rmresp'%args[1])         # where to store the downloaded data
...
rm_resp   = 'inv'                               # False to not remove, True to remove, but 'inv' to remove with inventory
...
if rank == 0:
    ...
    fo=open(DATADIR+"/../downloadtime_BP_removeresp_np%s.txt"%args[1], "w")
