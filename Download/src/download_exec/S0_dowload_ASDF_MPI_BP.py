...
stalist  = os.path.join(rootpath,'request_BP.csv')      # CSV file for station location info
...
NewFreq   = 20.0                                  # resampling at X samples per seconds
...
out_form  = 'MSEED'                              # choose between ASDF and SAC
...
end_date   = ["2006_06_01_0_0_0"]               # end date of download
    ...
    for ista in range(nsta):
        ...
        if len(tr):
            if out_form == 'ASDF':
                ...
                ff=os.path.join(DATADIR,net[ista]+sta[ista]+'.'+all_chunck[ick]+'T'+all_chunck[ick+1]+'.mseed')
                tr.write(ff, format="MSEED")
...
if rank == 0:
    ...
    fo=open(DATADIR+"/../downloadtime_BP_np%s.txt"%args[1], "w")
     # gather total download size
    fo.write('rank: %s size: %s\n'%(rank, size))
    fo.write("%12.8f [s]"%(tt1-tt0))
    fo.write("%12.4f [MB]\n"%(totaldownloadsize))
    fo.close()
    sys.exit()
