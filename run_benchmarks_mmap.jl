function run_benchmarks_mmap(n_0::Int64=1, n_1::Int64=0)
  N = size(tests,1)
  if n_1 == 0
    n_1 = N
  end
  ...
  for n = n_0:n_1
    ...
    if opt == "win"
      S = read_data(f_call, fname, cf=cfile, nx_new=360000, memmap=true)
      B = @benchmarkable(read_data($f_call, $fname, cf=$cfile, nx_new=360000, memmap=true),  overhead=ov)
      b = run(B)
    elseif opt == "lo-mem"
      S = read_data(f_call, fname, nx_new=nx_new, nx_add=nx_add, memmap=true)
      B = @benchmarkable(read_data($f_call, $fname, nx_new=$nx_new, nx_add=$nx_add, memmap=true), overhead=ov)
      b = run(B)
    elseif opt == "full"
      S = read_data(f_call, fname, full=true, memmap=true)
      B = @benchmarkable(read_data($f_call, $fname, full=true, memmap=true),  overhead=ov)
      b = run(B)
    elseif f_call == "asdf"
      ...
      B = @benchmarkable(read_data($f_call, $fname, memmap=true),  overhead=ov)
      b = run(B)
    end
